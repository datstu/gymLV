                                                    ...
                                                    <div class="service-desc">
                                                        ...
                                                        <ul>
                                                           ...
                                                           @if($value->id_user == $user->id_user )
                                                            <li> <a href="{{ route('booklich',$value->id_order) }}"><strong>Người sử dụng: {{$value->consignee_name}} </strong>&ensp;Ngày đăng ký: {{$value->order_date}}<br>Dịch vụ: {{$value->ten}}({{$value->date}} tháng)@if($value->HLV==1) - Có HLV riêng (Gói có HLV sẽ không được đổi thời khóa biểu. )@else - Gói không có HLV @endif <br> @if($value->id_gym)Thời hạn: {{$value->date_end}} &ensp;Ngày bắt đầu: {{$value->date_begin}}@else Chưa đặt lịch @endif<br>
                                                            Trạng thái: 
